const operator = {
  ...
  equalTo: (val, name) => val === document.querySelector(`input[name="${name}"]`).value,
  katakana: (val) => {
    if (!val) {
      return true;
    }
  ...
  all_justChecked: (checkboxes, num) => parseInt(num, 10) === checkboxes.filter(({ checked }) => checked).length,  
  all_minChecked: (checkboxes, num) => parseInt(num, 10) <= checkboxes.filter(({ checked }) => checked).length,  
  all_maxChecked: (checkboxes, num) => parseInt(num, 10) >= checkboxes.filter(({ checked }) => checked).length,  
  dates: (val) => {
    if (!val) {
      return true;
    }
