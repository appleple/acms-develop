...
class Validator {
  ...
  register() {
    ...
    this.eventRegistry.set(this.form, [
      { type: "submit", listener: this.handleSubmit }
    ])
  }
  ...
  validateAll() {
    ...
    const names = [
      ...new Set(fields.map(field => field.name.replace(/\[[\d]*\]/, "")))
    ]
    ...
    allValidations.forEach(validation => {
      ...
      const ok = this.rules[validation.rule](
        "",
        validation.arg,
        checkboxes,
        validation
